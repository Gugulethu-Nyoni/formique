(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.Formique = factory());
  ...
  `.replace(/^\s*\n/gm, '').trim();
	  ...
	  let formattedHtml = pretty$1(formHTML, {
	    indent_size: 2,
	    wrap_line_length: 0,
  ...
  `.replace(/^\s*\n/gm, '').trim();
	...
	renderRadioField(type, name, label, validate, attributes, bindingSyntax, options) {
	  ...
	  if (validate) {
	    Object.entries(validate).forEach(([key, value]) => {
	      if (radioValidationAttributes.includes(key)) ; else {
	        console.warn(`\x1b[31mUnsupported validation attribute '${key}' for field '${name}' of type '${type}'.\x1b[0m`);
	      }
	    });
	  }
  ...
  `.replace(/^\s*\n/gm, '').trim();
	...
	renderCheckboxField(type, name, label, validate, attributes, bindingSyntax, options) {
	  ...
	  if (validate) {
	    Object.entries(validate).forEach(([key, value]) => {
	      if (checkboxValidationAttributes.includes(key)) ; else {
	        console.warn(`\x1b[31mUnsupported validation attribute '${key}' for field '${name}' of type '${type}'.\x1b[0m`);
	      }
	    });
	  }
  ...
  `.replace(/^\s*\n/gm, '').trim();
	  ...
	  // Format the entire HTML using pretty
	  const formattedHtml = pretty$1(formHTML, {
	    indent_size: 2,
	    wrap_line_length: 0,
	    preserve_newlines: true, // Preserve existing newlines
	  });
