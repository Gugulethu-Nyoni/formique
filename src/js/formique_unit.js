...
renderSingleSelectField(type, name, label, validate, attributes, bindingSyntax, options, subCategoriesOptions, mode) {
    ...
    const onchangeAttr = (mode === 'dynamicSingleSelect' && subCategoriesOptions) ? ' onchange="handleDynamicSingleSelect(this.value)"' : '';
...
subCategoriesOptions.forEach(subCategory => {
  ...
  let formHTML = `
    <fieldset class="${this.selectGroupClass}" id="${id}-options" style="display: none;">
        <label for="${id}">${label} 
